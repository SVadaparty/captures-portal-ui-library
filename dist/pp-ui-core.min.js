...
window.PPDateComponent = (function () {
  ...
  function formatFromISO(iso, format) {
    if (!iso) return "";
  ...
  function initDatepicker() {
    const datepickers = document.querySelectorAll("[data-pp-datepicker]");
    ...
    datepickers.forEach(input => {
      ...
      if (pair) {
        ...
        groups[pair][input.dataset.role] = input;
      }
    });

    datepickers.forEach(input => {
      const onChangeFn = input.dataset.onchange;
      const format = input.dataset.format || "YYYY-MM-DD";
      const role = input.dataset.role;
      const pair = input.dataset.pair;

      input.type = DEFAULTS.dateInputType || "date";
      ...
      /* PAIR SYNC */
      if (pair && groups[pair]) {
        const { min, max } = groups[pair];
        if (min && max) {
          if (role === "min") max.min = todayISO;
          if (role === "max") min.max = todayISO;
        }
      }
      ...
      input.addEventListener("change", e => {
        ...
        if (pair && groups[pair]) {
          const { min, max } = groups[pair];
          if (min && max) {
            if (role === "min") max.min = isoValue;
            if (role === "max") min.max = isoValue;
          }
        }

        const formattedValue = formatFromISO(isoValue, format);

        if (onChangeFn && typeof window[onChangeFn] === "function") {
          window[onChangeFn](formattedValue, isoValue, e);
        }
      });
  ...
  /* =========================
     RADIO → DATE MODE CHANGE
  ========================= */
  function onDateModeChange(value) {
    const input = document.querySelector("[data-pp-datepicker]");
    if (!input) return;
    ...
    if (value === "Minimum date") {
      input.min = todayISO;   // disable past
    }

    if (value === "Maximum date") {
      input.max = todayISO;   // disable future
    }

    input.value = todayISO;

    /* 🔥 AUTO-OPEN CALENDAR AFTER APPLYING CONSTRAINTS */
    setTimeout(() => {
      if (typeof input.showPicker === "function") {
        input.showPicker();   // Chrome / Edge
      } else {
        input.focus();        // Safari / Firefox fallback
      }
    }, 50);
  }
