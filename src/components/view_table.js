...
function applyInputStyles(input, overrides = {}) {
  ...
  input.style.border = `1px solid ${overrides.borderColor || DEFAULTS.tableInputBorderColor
    }`;
...
/* ======================================================
   TABLE EXPORT EXTENSION (ADD-ON)
   ====================================================== */
window.PPTableExport = (function () {

  function getTableContainer() {
    return document.querySelector("[data-pp-table]");
  }

  function getDataFromSource() {
    const table = getTableContainer();
    if (!table) {
      console.warn("No data-pp-table found");
      return null;
    }

    const sourceName = table.dataset.datasource;
    const data = window[sourceName];

    if (!Array.isArray(data)) {
      console.warn(`Datasource '${sourceName}' not found on window`);
      return null;
    }
  ...
  /* ================= CSV ================= */
  function exportCSV() {
    const result = getHeadersAndRows();
    if (!result) return;
    ...
    download(csv, "table_export.csv", "text/csv");
  }
  ...
  function exportExcel() {
    ...
    download(html, "table_export.xls", "application/vnd.ms-excel");
  }
